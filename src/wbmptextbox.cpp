...
WBmpTextBox::~WBmpTextBox()
{
	...
	if (_oldproc)
	{
		SetWindowLongPtr(Handle, GWLP_WNDPROC, (LONG_PTR)_oldproc);
	}
...
int WBmpTextBox::SetText(char* text, BOOL fRedraw)
{
	...
	strcpy_s(_lpText, _nTextLen + 1, text);
	...
	WBmpFont* font)
{
	...
	strcpy_s(_scroll_text_delimiter, 5, " - ");
	...
	_oldproc = (WNDPROC)SetWindowLongPtr(Handle, GWLP_WNDPROC, (LONG_PTR)_WindowSubclassProc);
...
LRESULT CALLBACK WBmpTextBox::_WindowSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	WBmpTextBox* control = (WBmpTextBox*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
	...
	case WM_LBUTTONDOWN: {
		...
		LONG_PTR ButtonID = GetWindowLongPtr(control->Handle, GWLP_ID);
	...
	case WM_DESTROY:
		SetWindowLongPtr(control->Handle, GWLP_WNDPROC, (LONG_PTR)control->_oldproc);
...
int WBmpTextBox::SetScrollTextDelimiter(char* delimiter)
{
	...
	int _scroll_text_delimiter_len = (int)strlen(delimiter) + 1;
	_scroll_text_delimiter = (char*)malloc(_scroll_text_delimiter_len);
	...
	strcpy_s(_scroll_text_delimiter, _scroll_text_delimiter_len, delimiter);
	...
	*(text - 1) = first;
	...
	/*
		...
		if(strlen(text) > 1)
		{
			...
			sprintf_s(text,"%s%s",text+1,first);
		}
		*/
}
