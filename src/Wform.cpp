...
LRESULT CALLBACK WForm::WindowProcedure (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg) 
    {
        ...
        case WM_HSCROLL:
        	return form->OnHScroll((DWORD) GetWindowLong((HWND) lParam,GWL_ID),  (int) LOWORD(wParam), int HIWORD(wParam), (HWND) lParam);


        case WM_VSCROLL:
        	return form->OnVScroll((DWORD) GetWindowLong((HWND) lParam,GWL_ID), (int) LOWORD(wParam), int HIWORD(wParam), (HWND) lParam);
