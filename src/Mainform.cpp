	...
	case SB_SEEK:
		...
		case SB_THUMBTRACK: {
			...
			if (bElapsedTime)
			{
				...
				sprintf_s(text, "%02i:%02i:%02i", h, m, s);
			}
			else
			{
...
LRESULT MainForm::OnVScroll(DWORD ScrollbarID, int nScrollCode, int nPos, HWND scHandle)
{
	...
	case SB_EQ1: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 1: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
	...
	case SB_EQ2: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 2: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ3: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 3: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ4: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 4: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ5: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 5: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ6: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 6: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ7: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 7: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ8: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 8: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ9: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 9: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_EQ10: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq band 10: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_PREAMP: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Eq preamp: %i", (12000 - nPos) / 1000);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
			break;
		}
	...
	case SB_ECHO: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Echo gain: %i", 12 - sbEcho->GetPos());
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
		}
		else
	...
	case SB_INPUT: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Input gain: %i", 12 - sbInput->GetPos());
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
		}
		else
	...
	case SB_OUTPUT: {
		...
		{
			char tmp[20];
			sprintf_s(tmp, "Output gain: %i", 12 - sbOutput->GetPos());
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
		}
		else
	...
	case SB_DELAY: {
		if (nScrollCode == SB_THUMBTRACK)
		{
			...
			char tmp[20];
			sprintf_s(tmp, "Delay: %i ms", (20 - sbDelay->GetPos()) * 50);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
		}
	...
	case SB_LVOLUME: {
		if (nScrollCode == SB_THUMBTRACK)
		{
			...
			char tmp[20];
			sprintf_s(tmp, "Volume: %i", 100 - nPos);
			blockStatus = TRUE;
			tbSongStatus->SetText(tmp, 1);
		...
		/*
				case SC_MVOLUME:
					...
					if(nScrollCode == SB_THUMBTRACK) {
						...
						char tmp[20];
						sprintf_s(tmp,"Volume: %i", ( 655 - nPos) * 100 /655);
						blockStatus = TRUE;
...
LRESULT MainForm::OnTimer(UINT idTimer)
{
	...
	case MAIN_TIMER: {
		...
		sprintf_s(text, "%i", player->GetBitrate(0));
		...
		if (bElapsedTime)
			sprintf_s(text, "%02i:%02i:%02i", time.hms.hour, time.hms.minute, time.hms.second);
...
LRESULT MainForm::OnCommand(int wNotifyCode, int ControlID, HWND hWndControl)
{
	...
	case M_EXTRACT: {
		...
		if (dwSeek != 0)
		{
			...
			{
				strcpy_s(outfilename, savedlg->GetFileName());
				delete savedlg;
				...
				{
					sprintf_s(text, "Can't open file: %s for writing", outfilename);
	...
	case BT_OPEN: {
		...
		{
			strcpy_s(mp3filename, opendlg->GetFileName());
			delete opendlg;
	...
	case M_CONVERT: {
		...
		char tmp[2 * MAX_PATH];
		sprintf_s(tmp, "%s.exe", mp3filename);
		...
		{
			sprintf_s(text, "Can't open file: %s", exefilename);
			MessageBox(Handle, text, "Error", MB_ICONSTOP);
			delete exefile;
			delete outfile;
			break;
		...
		{
			sprintf_s(text, "Can't open file: %s", mp3filename);
			MessageBox(Handle, text, "Error", MB_ICONSTOP);
			delete mp3file;
			delete outfile;
			break;
...
BOOL Start(char* filename)
{
	...
	sprintf_s(listBoxEntry, "%s", fileTitle);

	TID3Info id3;
	if (player->LoadID3(id3Version2, &id3))
	{
		sprintf_s(listBoxEntry, "%s - %s", id3.Artist, id3.Title);
	}
	...
	sprintf_s(text, "%02i:%02i:%02i", info.Length.hms.hour, info.Length.hms.minute, info.Length.hms.second);
	tbSongLength->SetText(text, 1);

	sprintf_s(text, "%i", info.SamplingRate);
