...
int WBmpFont::SetText(char* lpText)
{
	...
	strcpy_s(_lpText, _dwTextLen + 1, lpText);
...
{
	if (!hbFont || !lpCharMap || dwFontHeight == 0)
		return 0;
	...
	_dwCharMapLen = (int)strlen(lpCharMap);
	_lpCharMap = (char*)malloc(_dwCharMapLen + 1);
	if (!_lpCharMap)
		return 0;

	strcpy_s(_lpCharMap, _dwCharMapLen + 1, lpCharMap);
