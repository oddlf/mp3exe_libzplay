...
{
	WBmpButton* bt = (WBmpButton*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
	...
	case WM_DESTROY:
		SetWindowLongPtr(bt->Handle, GWLP_WNDPROC, (LONG_PTR)bt->_oldproc);
...
void WBmpButton::ButtonUp(WBmpButton* bt, LPARAM lParam)
{
	...
	{ // in button rect
		...
		{ // button was pushed
			...
			LONG_PTR ButtonID = GetWindowLongPtr(bt->Handle, GWLP_ID);
