...
void WScrollbar::MouseMoveSC(WScrollbar* control, WPARAM wParam, LPARAM lParam)
{
	...
	if (control->_pushed)
	{
		...
		if (control->_interpos < 0)
			control->_interpos = 0;
		if (control->_interpos > (max - min - control->_thumbsize))
			control->_interpos = max - min - control->_thumbsize;
...
{
	WScrollbar* control = (WScrollbar*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
